...
public class HistoryAssignmentController extends CommonController<HistoryAssignment, HistoryAssignmentService> {
	...
	public ResponseEntity<?> editar(@Valid @RequestBody HistoryAssignment historyAssignment, BindingResult result,
		...
		historyAssignmentDB.setPatient(historyAssignment.getPatient());
		historyAssignmentDB.setEstado(historyAssignment.isEstado());
		historyAssignmentDB.setFechaInicio(historyAssignment.getFechaInicio());
		historyAssignmentDB.setFechaFin(historyAssignment.getFechaInicio());
...
/*
	...
	public ResponseEntity<?> findPatientById(@PathVariable Long id){
		HistoryAssignment assignment = service.findBedByPatientId(id);
